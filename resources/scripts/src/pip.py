...
class MacOSpip:
    ...
    def _pipinstall(self):
        ...
        value = subprocess.run([f'/usr/local/bin/python{major}.{minor}', '-m', 'pip', 
                                'install', '--target', f"{folder}/py-modules", "-r", 'requirements.txt', '--upgrade'], check=True)
        if value.returncode != 0:
            pd.logpost(3, "pip return value" + str(value))
            pd.error("You need to restart PureData, to check if the installation process worked")
        else:
...
def pipinstallRequirements(mypackage):
    """Install a Python package from Pd"""
    global folder
    global package
    package = mypackage
        ...
        if platform.system() == 'Linux':
            ...
            try:
                if pd.pd_has_gui():
                    ...
                    label.pack(fill="both", expand=1)
                    root.update()   
                value = subprocess.run([f'python{major}.{minor}', '-m', 'pip', 'install', 
                                        '--target', f"{folder}/py-modules", "-r", 'requirements.txt', 
                                        '--upgrade'], check=True)
                if value.returncode != 0:
                    pd.error("Some error occur with Pip.")
                    if root:
            ...
            # change all / to \
            folder = folder.replace('/', '\\')
            command = ['py', f'-{major}.{minor}', '-m', 'pip', 'install', '--target', f"{folder}", "-r", 'requirements.txt', '--upgrade']
            result = subprocess.run(command, check=False)
            if result.returncode != 0:
                pd.error("Some error occur with Pip.")
                return 'bang'
