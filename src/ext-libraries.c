...
void Py4pdLib_SetKwargs(t_py *x, t_symbol *s, int ac, t_atom *av) {
    ...
    if (av[0].a_type != A_SYMBOL) {
        pd_error(x,
                 "The first argument of the message 'kwargs' must be a symbol");
        return;
    }
    if (ac < 2) {
    ...
    if (ac == 2) {
        if (av[1].a_type == A_FLOAT) {
            int isInt =
                atom_getintarg(1, ac, av) == atom_getfloatarg(1, ac, av);
            if (isInt)
...
static int Py4pdLib_CreateObjInlets(PyObject *function, t_py *x, int argc,
    ...
    if (x->use_pArgs && defaultsCount > 0) {
        pd_error(x,
                 "[py4pd] You can't use *args and defaults at the same time");
        return -1;
    }
    ...
    if (pyFuncArgs != 0) {
        ...
        for (i = 0; i < pyFuncArgs; i++) {
            ...
            if (i + 1 >= indexWhereStartDefaults) {
                PyPtrValue->pValue = PyTuple_GetItem(
                    defaults, (i + 1) - indexWhereStartDefaults);
            } else {
        ...
        for (i = 0; i < argNumbers; i++) {
            if (i <= argc) {
                if (argv[i].a_type == A_FLOAT) {
                    ...
                    if (strcmp(atom_getsymbolarg(i, argc, argv)->s_name,
                               "None") == 0) {
                        Py_INCREF(Py_None);
                        x->pyObjArgs[i]->pValue = Py_None;
                    } else {
                        x->pyObjArgs[i]->pValue = PyUnicode_FromString(
...
static void Py4pdLib_Dsp(t_py *x, t_signal **sp) {
    ...
    if (x->objType == PY4PD_AUDIOINOBJ) {
        ...
        dsp_add(Py4pdLib_AudioINPerform, 3, x, sp[0]->s_vec,
                PY4PDSIGTOTAL(sp[0]));
    } else if (x->objType == PY4PD_AUDIOOUTOBJ) {
        x->vectorSize = sp[0]->s_n;
        signal_setmultiout(&sp[0], x->nChs);
        dsp_add(Py4pdLib_AudioOUTPerform, 3, x, sp[0]->s_vec,
                PY4PDSIGTOTAL(sp[0]));
...
PyObject *Py4pdLib_AddObj(PyObject *self, PyObject *args, PyObject *keywords) {
    ...
    if (keywords != NULL) {
        ...
        if (PyDict_Contains(keywords,
                            PyUnicode_FromString("require_outlet_n"))) {
            PyObject *output =
                PyDict_GetItemString(keywords, "require_outlet_n");
            if (output == Py_True) {
                require_outlet_n = 1;
            }
        }
        ...
        if (PyDict_Contains(keywords, PyUnicode_FromString("helppatch"))) {
            ...
            if (helpPatchLen >= suffixLen) {
                if (strcmp(helpPatch + (helpPatchLen - suffixLen), suffix) ==
                    0) {
                    personalisedHelp = 1;
        ...
        if (PyDict_Contains(keywords,
                            PyUnicode_FromString("ignore_none_return"))) {
            PyObject *noneReturn =
        ...
        if (PyDict_Contains(keywords,
                            PyUnicode_FromString("num_aux_outlets"))) {
    ...
    PyDict_SetItemString(nestedDict, "py4pdOBJpyout",
                         PyLong_FromLong(objpyout));
    ...
    PyDict_SetItemString(nestedDict, "py4pdOBJrequireoutletn",
                         Py_RequireOutletN);
    ...
    class_addmethod(localClass, (t_method)Py4pd_SetPythonPointersUsage,
                    gensym("pointers"), A_FLOAT, 0);
    class_addmethod(localClass, (t_method)Py4pdLib_ReloadObject,
                    gensym("reload"), 0, 0);
    ...
    if (py_args != 0) {
        ...
        class_addmethod(py4pdInlets_proxy_class,
                        (t_method)Py4pdLib_ProxyPointer, gensym("PyObject"),
                        A_SYMBOL, A_POINTER, 0);
    }
    if (added2pd_info == 1) {
