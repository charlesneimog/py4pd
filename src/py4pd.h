...
#define PYTHON_REQUIRED_VERSION(major, minor)                                  \
...
// ====================================
/*
 * @brief Structure representing the values that are stored in the dictionary to
 * be played for the player.
 */
typedef struct {
    int onset;
    int size;
...
typedef struct _py { // It seems that all the objects are some kind of class.
    ...
    t_int
        use_NumpyArray;  // flag to check if is to use numpy array in audioInput
    ...
    t_symbol *tempPath; // temp path located in ~/.py4pd/, always is deleted
                        // when py4pd is closed
...
typedef struct _py {
    ...
    t_int                 visualization_object; // 1 for canvas, 2 for picture,
3 for score t_int                 function_called; // flag to check if the set
function was called t_int                 argument_count; // number of arguments
    t_int                 output_py_pointer; // flag to check if it is to output
the Python pointer t_int                 use_kwargs;
    ...
    // Library
    t_int                 use_py4pd_library; // flag to check if it is to use
the Python library t_int                 is_py4pd_object; t_int ignore_none;
    ...
    // AUDIO AND NUMPY
    t_int                 use_audio_output; // flag to check if it is to use
audio output t_int                 use_audio_input; // flag to check if it is to
use audio input t_int                 use_numpy_array; // flag to check if it is
to use NumPy array in audio input t_int                 numpy_imported; // flag
to check if NumPy was imported t_float               audio_buffer; // audio
    ...
    t_int                 initialized; // flag to check if the object was
initialized t_int                 default_image; // flag to check if the object
was initialized t_int                 selected; // flag to check if the object
was selected t_int                 num_inlets; t_int num_outlets; t_int
mouse_over; t_symbol             *full_name; t_symbol             *file_name;
    t_symbol             *tcl_variable_x; // name of the Tcl variable for x
    char                 *image_data;
    t_symbol             *editor_name; // editor name

    // PATHS
    t_symbol             *package_path; // packages path, where the packages are
located t_symbol             *pd_patch_folder; // where the patch is located
    t_symbol             *py4pd_path; // where Py4pd object is located
    t_symbol             *temp_path; // temporary path located in ~/.py4pd/,
always deleted when Py4pd is closed t_symbol             *library_folder; //
...
// =====================================
t_py4pd_pValue *Py4pdUtils_Run(t_py *x, PyObject *pArgs,
                               t_py4pd_pValue *pValuePointer);
void *Py4pd_ImportNumpyForPy4pd();

// =====================================
// void Py4pd_SetParametersForFunction(t_py *x, t_symbol *s, int argc, t_atom
// *argv);
...
#define PY4PD_IMAGE                                                            \
    "R0lGODlhKgAhAPAAAP///"                                                    \
    "wAAACH5BAAAAAAAIf8LSW1hZ2VNYWdpY2sOZ2FtbWE9MC40NTQ1NDUALAAAAAAqACEAAAIkh" \
    "I+py+0Po5y02ouz3rz7D4biSJbmiabqyrbuC8fyTNf2jTMFADs="
...
// ============= UTILITIES =============
// int Py4pdUtils_ParseLibraryArguments(t_py *x, PyCodeObject *code, int argc,
// t_atom *argv);
int Py4pdUtils_ParseLibraryArguments(t_py *x, PyCodeObject *code, int *argc,
                                     t_atom **argv);
...
extern void Py4pdPic_InitVisMode(t_py *x, t_canvas *c, t_symbol *py4pdArgs,
                                 int index, int argc, t_atom *argv,
                                 t_class *obj_class);
...
extern void Py4pdPic_GetRect(t_gobj *z, t_glist *glist, int *xp1, int *yp1,
                             int *xp2, int *yp2);
...
// ============= EXTERNAL LIBRARIES =============
extern PyObject *Py4pdLib_AddObj(PyObject *self, PyObject *args,
                                 PyObject *keywords);
