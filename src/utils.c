  ...
  for (i = 0; i < argc; i++) {
    if (argv[i].a_type == A_SYMBOL) {
      ...
      if (py4pdArgs == gensym("-picture") || py4pdArgs == gensym("-score") ||
        ...
        x->use_NumpyArray = 0;
        x->out1 =
            outlet_new(&x->obj, gensym("signal")); // create a signal outlet
        int j;
        for (j = i; j < argc; j++) {
          argv[j] = argv[j + 1];
        }
...
// if it is linux
    ...
    sprintf(command, "emacs --eval '(progn (find-file \"%s\") (goto-line %d))'",
            completePath, line);
  } else {
...
void Py4pdUtils_DECREF(PyObject *pValue) {
  // if (pValue->ob_refcnt < 0){
  // pd_error(NULL, "[DEV] pValue refcnt < 0, Memory Leak, please
  // report!"); return;
  // }
...
PyObject *Py4pdUtils_AddPdObject(t_py *x) {
  ...
  if (MainModule != NULL) {
    ...
    if (objectCapsule != NULL) {
      ...
      objectCapsule =
          PyCapsule_New(x, "py4pd", NULL); // create a capsule to pass the
...
void Py4pdUtils_CreatePicObj(t_py *x, PyObject *PdDict,
  ...
  if (gifFile == NULL) {
    ...
    if (gifFileCHAR[0] == '.' && gifFileCHAR[1] == '/') {
      ...
      sprintf(completeImagePath, "%s%s", PyUnicode_AsUTF8(pyLibraryFolder),
              gifFileCHAR);
