...
void Py4pdUtils_ParseArguments(t_py *x, t_canvas *c, int argc, t_atom *argv) {
    ...
    if (x->audioOutput == 0) {
        x->out1 = outlet_new(
            &x->obj, 0); // cria um outlet caso o objeto nao contenha audio
    }
}
...
PyObject *Py4pdUtils_RunPyAudioOut(t_py *x, PyObject *pArgs,
    ...
    if (MainModule != NULL) {
        ...
        if (oldObjectCapsule != NULL) {
            ...
            prev_obj = (t_py *)PyCapsule_GetPointer(
                py4pd_capsule, "py4pd"); // borrowed reference
            prev_obj_exists = 1;
    ...
    if (MainModule != NULL) {
        ...
        if (objectCapsule != NULL) {
            ...
            objectCapsule = PyCapsule_New(
                x, "py4pd", NULL); // create a capsule to pass the object to
