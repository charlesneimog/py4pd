...
void Py4pdPlayer_PlayerSendNewThing(t_py *x, int onset, t_symbol *receiver, PyObject *value) {
    ...
    if (index != -1) {
        ...
        x->playerDict->entries[index].receiver =
            (t_symbol **)realloc(x->playerDict->entries[index].receiver,
                                 (x->playerDict->entries[index].size + 1) * sizeof(t_symbol *));

        x->playerDict->entries[index].values[x->playerDict->entries[index].size] = value;
        x->playerDict->entries[index].receiver[x->playerDict->entries[index].size] = receiver;
        ...
        newEntry->isReceiverPlayer = 1;
        newEntry->values = (PyObject **)malloc(sizeof(PyObject *));
        newEntry->values[0] = value;
        newEntry->receiver = (t_symbol **)malloc(sizeof(t_symbol *));
        newEntry->receiver[0] = receiver;
        newEntry->size = 1;
        x->playerDict->size++;
    }
}
