...
void *Py4pdLib_NewObj(t_symbol *s, int argc, t_atom *argv) {
    ...
    int inlets = Py4pdUtils_CreateObjInlets(pyFunction, x, py4pdInlets_proxy_class, argc, argv);
    if (inlets != 0) {
        free(x->pdObjArgs);
        free(x->pyObjArgs);
        return NULL;
...
PyObject *Py4pdLib_AddObj(PyObject *self, PyObject *args, PyObject *keywords) {
    ...
    // special methods
    if (objectType == PY4PD_NORMALOBJ) {
        class_addmethod(localClass, (t_method)Py4pdUtils_Click, gensym("click"), A_NULL, 0);
        if (playableInt == 1) {
            class_addmethod(localClass, (t_method)Py4pdPlayer_Play, gensym("play"), A_GIMME, 0);
            class_addmethod(localClass, (t_method)Py4pdPlayer_Stop, gensym("stop"), A_NULL, 0);
            class_addmethod(localClass, (t_method)Py4pdPlayer_Clear, gensym("clear"), A_NULL, 0);
        }
    } else if (objectType == PY4PD_VISOBJ) {
        if (playableInt == 1) {
            class_addmethod(localClass, (t_method)Py4pdPlayer_Play, gensym("play"), A_GIMME, 0);
    ...
    class_addmethod(localClass, (t_method)Py4pdLib_Pointer, gensym("PyObject"), A_SYMBOL, A_POINTER,
                    0);
    class_addmethod(localClass, (t_method)Py4pd_PrintDocs, gensym("doc"), A_NULL, 0);
    ...
    class_addmethod(localClass, (t_method)Py4pd_SetPythonPointersUsage, gensym("pointers"), A_FLOAT,
                    0);
    class_addmethod(localClass, (t_method)Py4pdLib_ReloadObject, gensym("reload"), A_NULL, 0);
    ...
    if (py_args != 0) {
        py4pdInlets_proxy_class = class_new(gensym("_py4pdInlets_proxy"), 0, 0,
