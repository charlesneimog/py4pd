...
static int Py4pdNewObj_Init(Py4pdNewObj *self, PyObject *args, PyObject *kwds) {
    ...
    // TODO: Fix to c++
    static char *keywords[] = {const_cast<char *>("param_str"), NULL};
    char *param_str;
...
void *Py4pdNewObj_NewObj(t_symbol *s, int argc, t_atom *argv) {
    ...
    PyObject *PdDictCapsule = (PyObject *)PyCapsule_GetPointer(py4pd_capsule, objectName);
    ...
    Py4pdNewObj *objClass = (Py4pdNewObj *)PyLong_AsVoidPtr(PyStructSelf);
    x->objClass = reinterpret_cast<Py4pdNewObject *>(objClass);
    ...
    x->pdObjArgs = (t_atom *)malloc(sizeof(t_symbol) * argc);
    for (int i = 0; i < argc; i++) {
        x->pdObjArgs[i] = argv[i];
    }
    x->objArgsCount = argc;
    if (x->pyObjArgs == NULL) {
        x->pyObjArgs = (t_py4pd_pValue **)malloc(sizeof(t_py4pd_pValue *) * x->pArgsCount);
    }
...
static PyObject *Py4pdNewObj_Method_AddObj(Py4pdNewObj *self, PyObject *args) {
    ...
    if (self->objIsPlayable) {
        ...
        class_addmethod(objClass, (t_method)Py4pdPlayer_Stop, gensym("stop"), A_NULL, 0);
        class_addmethod(objClass, (t_method)Py4pdPlayer_Clear, gensym("clear"), A_NULL, 0);
    }
    ...
    if (self->allowEdit) {
        class_addmethod(objClass, (t_method)Py4pdUtils_Click, gensym("click"), A_NULL, 0);
    }

    if (pArgCount != 0) {
        InletsExtClassProxy = class_new(gensym("_py4pdInlets_proxy"), 0, 0,
                                        sizeof(t_py4pdInlet_proxy), CLASS_DEFAULT, A_NULL);
...
PyTypeObject Py4pdNewObj_Type = {
    PyVarObject_HEAD_INIT(nullptr, 0) "pd.new_object", /* tp_name */
    sizeof(Py4pdNewObj),                               /* tp_basicsize */
    0,                                                 /* tp_itemsize */
    nullptr,                                           /* tp_dealloc */
    ...
    nullptr,                                           /* tp_iter */
    nullptr,                                           /* tp_iternext */
    Py4pdNewObj_methods,                               /* tp_methods */
    nullptr,                                           /* tp_members */
    Py4pdNewObj_GetSet,                                /* tp_getset */
    nullptr,                                           /* tp_base */
    nullptr,                                           /* tp_dict */
