...
void *thread_function(void *arg) {
    ...
    snprintf(command, sizeof(command),
             ...
             PY_MAJOR_VERSION, PY_MINOR_VERSION, pipArgs->package,
             install_path);
    post(command);
    system(command);
#endif
    ...
    if (keywords == NULL) {
        ...
        if (PyDict_Contains(keywords, PyUnicode_FromString("global_install"))) {
            PyObject *global_value =
                PyDict_GetItemString(keywords, "global_install");
            if (global_value == Py_True) {
                global = 1;
            }
        }
        if (PyDict_Contains(keywords, PyUnicode_FromString("detach_install"))) {
            PyObject *detach_value =
                PyDict_GetItemString(keywords, "detach_install");
