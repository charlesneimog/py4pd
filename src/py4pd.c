...
static int Py4pd_LibraryLoad(t_py *x, int argc, t_atom *argv) {
    ...
    if (access(script_file_path, F_OK) == -1 &&
        ...
        for (int i = 0; 1; i++) {
            ...
            char *library_path =
                malloc(strlen(pathelem) + strlen(script_file_name->s_name) + 2);
    ...
    if (!thereIsRequirements) {
        ...
        snprintf(localRequirements, localRequirementsSize,
                 "%s/requirements.txt", x->pdPatchFolder->s_name);
        ...
        if (argv[i].a_type == A_SYMBOL) {
            ...
            if (py4pdArgs == gensym("-picture") ||
                ...
                if (argv[i + 1].a_type == A_FLOAT &&
                    ...
                    pd_error(NULL,
                             "[py4pd] -picture, -score, -pic and -canvas was "
                             "removed in version 0.8.0");
                    pd_error(
                        NULL,
                        "[py4pd] Please transfor your code in one Pd Object, "
                        "it is simple");
                    pd_error(
                        NULL,
                        "[py4pd] Check: "
