...
static int Py4pd_LibraryLoad(t_py* x, int argc, t_atom* argv)
{
    ...
    pModule = PyImport_ImportModule(scriptFileName->s_name); // Import the script file
    if (pModule == NULL) {
        ...
        if (argc == 1) {
            if (argv[0].a_type == A_SYMBOL) {
                ...
                if (path->s_name[0] == '.' && path->s_name[1] == '/') {
                    char* new_path = malloc(strlen(x->pdPatchPath->s_name) + strlen(path->s_name) + 1);
...
void Py4pd_PrintDocs(t_py* x)
{
    ...
    if (x->pFunction && PyCallable_Check(x->pFunction)) { // Check if the function exists and is callable
...
void Py4pd_SetEditor(t_py* x, t_symbol* s, int argc, t_atom* argv)
{
    ...
    if (argc != 0 && argv[0].a_type == A_SYMBOL && strcmp(argv[0].a_w.w_symbol->s_name, "float") != 0) {
