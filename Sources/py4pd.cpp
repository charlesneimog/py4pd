...
static int Py4pd_LibraryLoad(t_py *x, int argc, t_atom *argv) {
    ...
    std::string LibName = atom_gensym(argv + 1)->s_name;
    ...
    if (!std::filesystem::exists(LibPathGlobal) && !std::filesystem::exists(LibPathLocal)) {
    ...
    t_py *PrevObj = nullptr;
    bool ThereIsPrevObj = false;
    ...
    if (pModule == nullptr) {
        pd_error(x, "[Python] Failed to load script file %s", LibName.c_str());
        Py4pdUtils_PrintError(x);
        Py_XDECREF(pModule);
        Py_XDECREF(MainModule);
        return -1;
    ...
    if (pFunc && PyCallable_Check(pFunc)) {
        ...
        if (ThereIsPrevObj && pValue != nullptr) {
        ...
        x->pModule = pModule;
        x->pFunction = pFunc;
        x->pScriptName = gensym(LibName.c_str());
        ...
        Py_XDECREF(pModuleReloaded);
        Py_XDECREF(pValue);
        logpost(x, 3, "[py4pd] Library %s loaded!", LibName.c_str());
    } else {
...
void Py4pd_SetFunction(t_py *x, t_symbol *s, int argc, t_atom *argv) {
    ...
    char *extension = (char *)strrchr(sScriptFilename->s_name, '.');
    if (extension != NULL) {
...
static void *Py4pd_Py4pdNew(t_symbol *s, int argc, t_atom *argv) {
    if (!Py_IsInitialized()) {
        ...
        if (PyImport_AppendInittab("pd", PyInit_pd) == -1) {
            pd_error(nullptr, "Error: could not extend in-built modules table");
            return nullptr;
        }
        Py_Initialize();
    }
    ...
    if (normalMODE == 1) {
        ...
        if (patchDir->s_name[strlen(patchDir->s_name) - 1] != '/') {
            char *new_path = (char *)malloc(strlen(patchDir->s_name) + 2); // TODO: Update
...
void py4pd_setup(void) {
    ...
    class_addmethod(py4pd_class, (t_method)Py4pd_SetPythonPointersUsage, gensym("pointers"),
                    A_FLOAT,
                    0); // set home path
    class_addmethod(py4pd_class, (t_method)Py4pd_ReloadPy4pdFunction, gensym("reload"), A_NULL,
    ...
    // Object INFO
    class_addmethod(py4pd_class, (t_method)Py4pd_PrintPy4pdVersion, gensym("version"), A_NULL,
                    0); // show version
    ...
    class_addmethod(py4pd_class, (t_method)Py4pd_SetEditor, gensym("click"), A_NULL,
                    0); // when click open editor
    class_addmethod(py4pd_classLibrary, (t_method)Py4pd_SetEditor, gensym("click"), A_NULL,
                    0); // when click open editor

    // User
    class_addmethod(py4pd_class, (t_method)Py4pd_PrintDocs, gensym("doc"), A_NULL,
