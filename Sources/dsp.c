...
static void Py4pdAudio_Audio2PdAudio(t_py *x, PyObject *pValue, t_sample *audioOut, int numChannels,
                                     int n) {
    if (pValue != NULL) {
        if (PyArray_Check(pValue)) {
            ...
            if (arrayLength <= n) {
                ...
                pd_error(x,
                         ...
                         "returned %d channels",
                         x->ObjName->s_name, numChannels, (int)arrayLength / (int)x->VectorSize);
                x->AudioError = 1;
            }
            Py_DECREF(pArray);
        } else {
            pd_error(x, "[%s] Python function must return a numpy array", x->ObjName->s_name);
            x->AudioError = 1;
        }
    } else {
        ...
        pd_error(x, "[%s] Call failed: %s", x->ObjName->s_name, PyUnicode_AsUTF8(pstr));
        Py_DECREF(pstr);
        Py_XDECREF(ptype);
        Py_XDECREF(pvalue);
...
void Py4pdAudio_Dsp(t_py *x, t_signal **sp) {
    if (x->ObjType == PY4PD_AUDIOINOBJ) {
        ...
        dsp_add(Py4pdAudio_AudioINPerform, 3, x, sp[0]->s_vec, PY4PDSIGTOTAL(sp[0]));
    } else if (x->ObjType == PY4PD_AUDIOOUTOBJ) {
        x->VectorSize = sp[0]->s_n;
        signal_setmultiout(&sp[0], x->nChs);
        dsp_add(Py4pdAudio_AudioOUTPerform, 3, x, sp[0]->s_vec, PY4PDSIGTOTAL(sp[0]));
    } else if (x->ObjType == PY4PD_AUDIOOBJ) {
        x->nChs = sp[0]->s_nchans;
        x->VectorSize = sp[0]->s_n;
        signal_setmultiout(&sp[1], sp[0]->s_nchans);
        dsp_add(Py4pdAudio_AudioPerform, 4, x, sp[0]->s_vec, sp[1]->s_vec, PY4PDSIGTOTAL(sp[0]));
    }
}
