...
void Py4pdUtils_FromSymbolSymbol(t_py *x, t_symbol *s, t_outlet *outlet) {
    ...
    seplen++;
    char *sep = (char *)t_getbytes(seplen * sizeof(*sep));
    memset(sep, '\0', seplen);
    Py4pdUtils_Strlcpy(sep, " ", seplen);
    if (s) {
        long unsigned int iptlen = strlen(s->s_name);
        t_atom *out = (t_atom *)t_getbytes(iptlen * sizeof(*out));
        iptlen++;
        char *newstr = (char *)t_getbytes(iptlen * sizeof(*newstr));
...
PyObject *Py4pdUtils_PointerToPyObject(PyObject *p) {
    PyObject *pValue = (PyObject *)PyLong_AsVoidPtr(p);
    return pValue;
}
...
void Py4pdUtils_CreatePicObj(t_py *x, PyObject *PdDict, t_class *object_PY4PD_Class, int argc,
    ...
    if (gifFile == NULL) {
        x->imageBase64 = const_cast<char *>(PY4PD_IMAGE);
    } else {
